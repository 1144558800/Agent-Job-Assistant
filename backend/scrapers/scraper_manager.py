...
class ScraperManager:
    ...
    async def search_all(self, keyword: str, city: str = "", page: int = 1, platforms: Optional[List[str]] = None) -> List[JobItem]:
        ...
        # 并行执行所有平台的搜索
        # 注意：在 sandbox 环境中事件循环不支持 create_subprocess_exec，
        # 因此通过 run_in_executor 在线程池中运行 Playwright，线程内部使用独立的 asyncio.run() 创建新事件循环
        ...
        # 搜索结果按城市过滤（各平台城市参数格式不统一，搜索后统一过滤）
        if city and all_jobs:
            before_filter = len(all_jobs)
            filtered_jobs = []
            city_keywords = [city]
            # 处理"北京"->"北京市"、"南京"->"南京市" 等简称与全称匹配
            if city == "北京":
