...
async def auto_apply_jobs(config: RunnableConfig, max_count: int = 5, match_threshold: float = 0.5) -> str:
    ...
    async def _apply_one(score: float, job) -> dict:
        ...
        if not scraper_key:
            return {
                ...
                "match_score": round(score, 2),
                "status": "failed",
                "reason": f"不支持的平台: {job.platform}"
            }
        
        scraper = mgr.scrapers.get(scraper_key)
        if not scraper:
            return {
                "platform": job.platform,
                "job_name": job.job_name,
                ...
                "match_score": round(score, 2),
                "status": "failed",
                "reason": f"爬虫未初始化: {scraper_key}"
            }
        
        try:
            # 线程隔离运行，带自动重试（浏览器自动化容易因网络/页面加载超时）
            loop = asyncio.get_running_loop()
            
            async def _do_send():
                if job.platform == "BOSS直聘":
                    job_id = job.platform_job_id
                    def _send():
                        return asyncio.run(scraper.send_greeting(job_id, greeting))
                else:
                    job_url = job.job_url
                    def _send():
                        return asyncio.run(scraper.send_greeting(job_url, greeting))
                return await loop.run_in_executor(None, _send)
            
            result = await _retry_tool(f"apply_{job.platform}", _do_send)
            
            # 记录投递历史
            is_success = result.get("success", False)
            record_apply(
                str(platform_id), job.platform, job.job_name, job.company_name,
                greeting, is_success, result.get("message", "")
            )
            
            # Guardrails 投递后记录（频率计数 + 审计日志）
            guard = get_guardrail_manager()
            guard.post_apply_record(is_success, job.platform)
            ...
            logger.error("[Tools][auto_apply] 投递异常 [{}]: {}", job.platform, e)
            return {
                "platform": job.platform,
                "job_name": job.job_name,
                "company": job.company_name,
    ...
    # 分离 BOSS直聘 和其他平台候选
    boss_candidates = [(s, j) for s, j in candidates if j.platform == "BOSS直聘"]
    other_candidates = [(s, j) for s, j in candidates if j.platform != "BOSS直聘"]
    
    logger.info("[Tools][auto_apply] BOSS直聘候选={}, 其他平台候选={}", len(boss_candidates), len(other_candidates))
    
    # 先并行投递其他平台（猎聘、智联等不受严格风控）
    if other_candidates:
        logger.info("[Tools][auto_apply] 开始投递其他平台（并行）...")
        other_tasks = [_apply_one(s, j) for s, j in other_candidates]
        other_results = await asyncio.gather(*other_tasks, return_exceptions=True)
        for r in other_results:
            if isinstance(r, Exception):
                results.append({"status": "failed", "reason": str(r)})
            else:
                results.append(r)
    
    # BOSS直聘串行投递（加上冷却间隔，防风控）
    if boss_candidates:
        logger.info("[Tools][auto_apply] 开始投递BOSS直聘（串行+冷却，防风控）...")
        BOSS_COOLDOWN_SEC = 90  # BOSS直聘每次投递间隔至少90秒（已触发过风控需要更保守）
        
        for idx, (s, j) in enumerate(boss_candidates):
            if idx > 0:
                cooldown = BOSS_COOLDOWN_SEC + random.randint(-5, 10)
                logger.info("[Tools][auto_apply] BOSS直聘冷却中: {:.1f}秒 (第{}/{})", cooldown, idx + 1, len(boss_candidates))
                await asyncio.sleep(cooldown)
            
            logger.info("[Tools][auto_apply] BOSS直聘投递 第{}/{}: {} | {}", 
                       idx + 1, len(boss_candidates), j.job_name, j.company_name)
            
            try:
                r = await _apply_one(s, j)
                results.append(r)
            except Exception as e:
                results.append({"status": "failed", "reason": str(e)})
                logger.error("[Tools][auto_apply] BOSS直聘投递异常 第{}: {}", idx + 1, e)
                
                # 如果被风控，跳过后续BOSS直聘候选
                if "风控" in str(e) or "异常行为" in str(e) or "禁止" in str(e):
                    logger.warning("[Tools][auto_apply] BOSS直聘可能被风控，跳过剩余 {} 个候选", 
                                 len(boss_candidates) - idx - 1)
                    for remaining in boss_candidates[idx + 1:]:
                        results.append({
                            "platform": remaining[1].platform,
                            "job_name": remaining[1].job_name,
                            "company": remaining[1].company_name,
                            "match_score": round(remaining[0], 2),
                            "status": "skipped",
                            "reason": "BOSS直聘风控，暂停投递"
                        })
                    break
    
    logger.info("[Tools][auto_apply] 投递完成: 成功={}, 跳过={}, 失败={}", 
               applied_count, skipped_count, len(results) - applied_count - skipped_count)
    
    # Hermes 事件驱动：连续投递失败自动触发反思
    total_failed = len(results) - applied_count - skipped_count
    if total_failed >= 3:
        try:
            hm = get_hermes_memory()
            logger.warning("[Tools][auto_apply] 投递失败数({})较多，触发连续失败检测", total_failed)
            triggered = hm.check_consecutive_failures(action_type="apply", threshold=3)
            if triggered:
                logger.info("[Tools][auto_apply] Hermes 事件驱动反思已触发")
        except Exception as e:
            logger.warning("[Tools][auto_apply] Hermes 连续失败检测异常: {}", e)
    
    # 按平台统计，并压缩 results 只保留关键字段（节省上下文 tokens）
    by_platform = {}
    compressed_results = []
    for r in results:
        p = r.get("platform", "未知")
        if p not in by_platform:
            by_platform[p] = {"success": 0, "failed": 0, "skipped": 0}
        status = r.get("status", "failed")
        by_platform[p][status] = by_platform[p].get(status, 0) + 1
        # 只保留关键字段，丢弃详细信息以节省上下文
        compressed_results.append({
            ...
            "status": status,
            "reason": r.get("reason", "")[:60] if r.get("reason") else "",
        })
    
    return json.dumps({
        "success": True,
        "message": f"自动投递完成：成功 {applied_count} 个，跳过 {skipped_count} 个，失败 {len(results) - applied_count - skipped_count} 个",
...
@tool
async def check_apply_history() -> str:
    """查看投递历史记录和统计信息。"""
    logger.info("[Tools][check_apply_history] 查询投递历史...")
    stats = get_apply_statistics()
...
@tool
async def reflect_on_experience() -> str:
    """
    触发 AI 反思分析：基于历史操作经验，DeepSeek AI 将分析搜索、投递、招呼语等策略效果，
    生成优化建议。使用此工具前应确保已积累一定数量的操作经验（搜索/投递等）。
    
    不需要参数。
    """
    logger.info("[Tools][reflect] === Hermes 反思触发 ===")
    ...
    try:
        ...
        if result.get("success"):
            new_count = len(result.get("new_insights", []))
            ...
            return json.dumps({
                "success": True,
                "message": f"反思完成！基于 {result['experience_count']} 条经验生成 {new_count} 条新洞察。{summary}",
                "experience_count": result["experience_count"],
                "new_insights_count": new_count,
                "summary": summary,
        ...
        logger.error("[Tools][reflect] 反思失败: {}", e)
        return json.dumps({"success": False, "message": f"反思失败: {str(e)}"}, ensure_ascii=False)


@tool
async def get_experience_insights(category: Optional[str] = None) -> str:
    """
    获取 Hermes 策略洞察：查看基于历史经验分析得出的优化建议。
    
    参数:
        category: 可选，洞察分类，可选值: "search_strategy"（搜索策略）, "apply_strategy"（投递策略）,
                 "greeting_style"（招呼语风格）, "platform_preference"（平台偏好）, "timing"（时机选择）。
                 不指定则返回全部。
    """
    logger.info("[Tools][insights] 查询洞察: category={}", category)
    ...
    try:
        ...
        # 获取活跃洞察
        insights = hm.get_active_insights(category)
        
        if not insights:
            return json.dumps({
                "success": True,
                "message": "暂无策略洞察。积累更多操作经验后（5条以上），可以说【进行反思总结】来生成优化建议。",
                "insights": [],
                "statistics": stats,
            }, ensure_ascii=False)
        ...
        return json.dumps({
            "success": True,
            "message": f"共 {len(insights)} 条活跃洞察",
            "insights": simple_insights,
            "statistics": stats,
        }, ensure_ascii=False)
    except Exception as e:
        logger.error("[Tools][insights] 查询洞察失败: {}", e)
        return json.dumps({"success": False, "message": f"查询失败: {str(e)}"}, ensure_ascii=False)


@tool
async def record_user_feedback(feedback: str, note: Optional[str] = None) -> str:
    """
    记录用户对最近操作的反馈，帮助系统更好地学习用户的偏好。
    
    参数:
        feedback: 反馈类型，可选值: "positive"（效果不错/有回复）, "negative"（效果不好/没回复）, "neutral"（一般）
        note: 可选，具体的反馈说明，如 "BOSS直聘上有2个HR回复了"
    """
    logger.info("[Tools][feedback] 用户反馈: feedback={}, note={}", feedback, note)
    
    try:
        hm = get_hermes_memory()
        
        # 获取最近的经验记录
        recent = hm.get_recent_experiences(count=5)
        
        if not recent:
            return json.dumps({
                "success": False,
                "message": "没有找到最近的操作经验，请先使用搜索或投递功能。",
            }, ensure_ascii=False)
        
        # 对最近的经验记录添加反馈
        applied_count = 0
        for exp in recent:
            if exp.get("user_feedback") is None:
                hm.record_user_feedback(
                    exp["record_id"],
                    feedback,
                    note or "",
                )
                applied_count += 1
                break  # 只给最近一条未标记的经验添加反馈
        
        if applied_count == 0:
            return json.dumps({
                "success": True,
                "message": "最近的操作都已有反馈记录。反馈未被重复添加。",
                "recent_count": len(recent),
            }, ensure_ascii=False)
        
        return json.dumps({
            "success": True,
            "message": f"反馈已记录（{feedback}）。系统会根据您的反馈持续优化策略。",
            "feedback": feedback,
        }, ensure_ascii=False)
    except Exception as e:
        logger.error("[Tools][feedback] 记录反馈失败: {}", e)
        return json.dumps({"success": False, "message": f"记录反馈失败: {str(e)}"}, ensure_ascii=False)
...
async def polish_resume(
    ...
    polish_focus: str = "project",
    output_format: str = "docx",
) -> str:
    """
    使用 AI 润色优化已上传的简历，重点让项目经历和工作经历看起来更专业。
    润色完成后生成新的 Word 或 PDF 文件供下载。

    参数:
        polish_focus: 润色重点，可选:
            - "project": 重点优化项目经历（默认），使用STAR法则重写项目描述
            - "work": 重点优化工作经历，突出职责和成果
            - "all": 全面润色整份简历
        output_format: 输出文件格式，可选 "docx"（Word）或 "pdf"（PDF），默认 "docx"

    使用前需确保已上传简历文件（PDF/Word/TXT格式均可）。
    """
    thread_id = config.get("configurable", {}).get("thread_id", "default") if config else "default"
    logger.info("[Tools][polish_resume] === 开始简历润色 ===")
    logger.info("[Tools][polish_resume] 参数: polish_focus={}, output_format={}, thread={}",
                polish_focus, output_format, thread_id)
    ...
    try:
        resume_file = resume_files[0]  # 使用最新的简历
        logger.info("[Tools][polish_resume] 简历文件: {}", resume_file.name)
        ...
        original_stem = resume_file.stem

        if file_suffix == ".docx":
            # ========== docx 文件：原地编辑，保留原始格式 ==========
            output_path = UPLOAD_DIR / f"{original_stem}_润色版_{timestamp}.docx"

            async def _do_polish_docx():
                return polish_docx_inplace(
                    input_path=str(resume_file),
            ...
            result_path = await _retry_tool("polish_resume", _do_polish_docx)
            logger.info("[Tools][polish_resume] docx 原地润色完成: {}", result_path)

            # 读取润色后的文本用于摘要
            resume_text = extract_resume_text(str(resume_file))
            polished_text = extract_resume_text(str(output_path))
            polish_summary = ""
            if resume_text and polished_text:
                # 只展示有变化的部分摘要
                polish_summary = f"原始 {len(resume_text)} 字符 -> 润色后 {len(polished_text)} 字符"

        elif file_suffix == ".pdf":
            # ========== PDF 文件：提取文本润色 -> 生成 docx 输出 ==========
            resume_text = extract_resume_text(str(resume_file))
            if not resume_text:
                return json.dumps({
                    "success": False,
                    "message": f"PDF 文件 {resume_file.name} 解析失败。"
                }, ensure_ascii=False)

            async def _do_polish_pdf():
                return polish_resume_with_ai(
            ...
            polished_text = await _retry_tool("polish_resume", _do_polish_pdf)
            output_path = UPLOAD_DIR / f"{original_stem}_润色版_{timestamp}.docx"
            generate_polished_docx(polished_text, str(output_path))
            polish_summary = polished_text[:800] + ("..." if len(polished_text) > 800 else "")

        else:
            # ========== TXT 文件：纯文本润色 -> 生成 docx ==========
            resume_text = extract_resume_text(str(resume_file))
            if not resume_text:
                return json.dumps({
                    "success": False,
                    "message": f"文件 {resume_file.name} 解析失败。"
                }, ensure_ascii=False)

            async def _do_polish_txt():
                return polish_resume_with_ai(
                    resume_text=resume_text,
                    client=qa.client,
                    model=qa.model,
            ...
            polished_text = await _retry_tool("polish_resume", _do_polish_txt)
            output_path = UPLOAD_DIR / f"{original_stem}_润色版_{timestamp}.docx"
            generate_polished_docx(polished_text, str(output_path))
            polish_summary = polished_text[:800] + ("..." if len(polished_text) > 800 else "")
        ...
        try:
            hm = get_hermes_memory()
            hm.record_experience(
                action_type="polish_resume",
                success=True,
                result_summary=f"简历润色完成: {resume_file.name} -> {output_path.name}",
                input_params={
                    "resume_file": resume_file.name,
                    "polish_focus": polish_focus,
            ...
            logger.warning("[Tools][polish_resume] Hermes 经验记录失败: {}", e)

        format_label = "保留原始格式的" if file_suffix == ".docx" else ""
        return json.dumps({
            "success": True,
            "message": f"简历润色完成！已生成{format_label}文件: {output_path.name}",
            "output_file": str(output_path),
            "output_filename": output_path.name,
            "polish_focus": polish_focus,
            "polish_summary": polish_summary[:600] if polish_summary else "",
        }, ensure_ascii=False)

    except Exception as e:
        logger.error("[Tools][polish_resume] 润色失败: {}", e)
        import traceback
        logger.error("[Tools][polish_resume] 异常堆栈:\n{}", traceback.format_exc())
        return json.dumps({
            "success": False,
            "message": f"简历润色失败: {str(e)}"
        }, ensure_ascii=False)


# ============================================================
# 桌面自动化工具 - 通过截图+视觉模型控制真实浏览器
# ============================================================

@tool
async def auto_communicate(
    keyword: str,
    city: str,
    config: RunnableConfig,
    salary_min: int = 0,
    ...
    """
    在招聘网站上自动搜索岗位、AI匹配简历、然后自动沟通。
    工作流程：搜索关键词 → 读取搜索结果 → AI判断简历与岗位匹配度 → 匹配度达标则自动点击"沟通"按钮发送招呼语。
    
    本工具在网站上实时搜索，不使用FAISS中存储的旧URL。

    参数:
        keyword: 搜索关键词，如 "AI开发工程师"、"Python开发"
        city: 城市名称，如 "苏州"、"北京"
        salary_min: 最低月薪（单位K），如 15 表示月薪不低于15K。0表示不限。
        platform: 在哪个招聘平台操作，默认 "51job"（前程无忧）。
                  也支持 "boss"（BOSS直聘）、"liepin"（猎聘）、"zhaopin"（智联招聘）
        max_jobs: 最多处理的岗位数量（默认5个）
        duration_minutes: 最长运行时间（默认30分钟，到时间自动停止）
        match_threshold: 简历匹配度阈值（默认0.8即80%），低于此阈值的不沟通
        greeting_style: 招呼语风格，"professional"（专业正式）、"casual"（轻松友好）、"short"（简短直接）

    重要:
        - keyword 和 city 必须提供，不能为空
        - 操作期间不要碰鼠标键盘，鼠标移至屏幕左上角可紧急停止
        - 浏览器当前标签页会被跳转到招聘网站的搜索页面
        - 简历从上传目录自动读取，不需要用户手动指定
    """
    thread_id = config.get("configurable", {}).get("thread_id", "default") if config else "default"
    logger.info("[Tools][auto_communicate] === 启动搜索匹配自动化 ===")
    logger.info("[Tools][auto_communicate] keyword={}, city={}, salary_min={}K, platform={}, match>={}%",
                keyword, city, salary_min, platform, int(match_threshold*100))
    ...
    try:
        ...
        try:
            ...
            if resume_files:
                ...
                logger.info("[Tools][auto_communicate] 读取简历: {}", resume_path.name)
                resume_text = extract_resume_text(resume_path)
                if resume_text:
                    logger.info("[Tools][auto_communicate] 简历文本长度: {} 字符", len(resume_text))
                else:
                    logger.warning("[Tools][auto_communicate] 简历文本为空")
        except Exception as e:
            logger.warning("[Tools][auto_communicate] 读取简历失败: {}", e)

        if not resume_text:
            logger.warning("[Tools][auto_communicate] 无简历文本，无法进行AI匹配")
            return json.dumps({
                "success": False,
                "message": "未找到简历文件。请先在左侧上传您的简历（PDF/Word/TXT），然后再启动自动沟通功能。没有简历，Agent无法判断岗位匹配度，所有的岗位都只能跳过。",
            }, ensure_ascii=False)

        # 1. 初始化桌面控制器
        from openai import OpenAI
        from config import AI_API_KEY, AI_API_BASE
        from agent.desktop_controller import (
            DesktopController, JobSearchMatchPipeline, PipelineState
        )
        controller_client = OpenAI(api_key=AI_API_KEY, base_url=AI_API_BASE)
        ctrl = DesktopController(client=controller_client, model="deepseek-chat")

        # 2. 检查浏览器窗口
        win = ctrl.find_browser_window()
        if not win:
            return json.dumps({
                "success": False,
                "message": "未找到浏览器窗口。请先打开浏览器（搜狗/Chrome/Edge等）并确保在前台可见。",
            }, ensure_ascii=False)
        browser_title = win.title[:50] if win else "未知"
        logger.info("[Tools][auto_communicate] 浏览器窗口: {}", browser_title)

        # 3. 准备招呼语模板
        greeting_templates = {
            "professional": "您好，我对贵司的'{title}'岗位很感兴趣。我有丰富的大模型和AI开发经验，与岗位要求高度匹配，期待您的回复。",
            "casual": "hi～看到你们在招'{title}'，我正好是做这个方向的，聊一下呗？",
            "short": "您好，应聘{title}，有相关经验，期待沟通。",
        }
        template = greeting_templates.get(greeting_style, greeting_templates["professional"])

        # 4. 创建搜索匹配流水线并启动
        state = PipelineState()
        pipeline = JobSearchMatchPipeline(ctrl, state)
        ...
        # 等待流水线完成（最长等待 duration_minutes + 60秒）
        timeout = duration_minutes * 60 + 60
        pipeline_thread.join(timeout=timeout)

        # 流水线结束后，获取报告
        report = state.get_report()
        logger.info("[Tools][auto_communicate] 流水线结束，报告已生成")

        return json.dumps({
            "success": True,
            "message": f"搜索匹配自动化已完成！\n\n{report}",
            "details": {
                "keyword": keyword,
                "city": city,
                "salary_min": salary_min,
                ...
                "job_results": [{
                    ...
                    "status": r.status,
                    "greeting_sent": r.greeting_sent,
                } for r in state.job_results],
            }
        }, ensure_ascii=False)

    except Exception as e:
        logger.error("[Tools][auto_communicate] 失败: {}", e)
        import traceback
        logger.error("[Tools][auto_communicate] 异常堆栈:\n{}", traceback.format_exc())
        return json.dumps({
            "success": False,
            "message": f"启动自动化失败: {str(e)}",
        }, ensure_ascii=False)
